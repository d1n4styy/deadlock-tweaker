...
from PyQt6.QtWidgets import (
    ...
    QSlider, QComboBox, QLineEdit, QStackedWidget, QSpacerItem,
    QGraphicsDropShadowEffect, QGraphicsOpacityEffect, QDialog, QFileDialog,
)
from PyQt6.QtCore import Qt, QSize, QTimer, QThread, QObject, pyqtSignal, QPropertyAnimation, QEasingCurve, QElapsedTimer, QRect
...
UPDATE_CHECK_URL = "https://raw.githubusercontent.com/d1n4styy/deadlock-tweaker/main/version.json"
...
# ──────────────────────────────────────────────────────────────────────────────
# Theme Toggle — sidebar pill (MDL2 icons, animated knob)
# ──────────────────────────────────────────────────────────────────────────────
class ThemeToggle(QWidget):
    clicked = pyqtSignal()

    _W      = 180   # fits inside 220px sidebar with 20px side margins
    _H      = 36
    _IW     = 34    # icon zone width each side
    _KD     = 26    # knob diameter
    _PAD    = 5     # vertical padding

    def __init__(self, parent=None):
        super().__init__(parent)
        self._checked = (_CURRENT_THEME == "light")
        self._anim_pos = 1.0 if self._checked else 0.0
        self._anim_from = self._anim_pos
        self._anim_elapsed = QElapsedTimer()
        self.setFixedSize(self._W, self._H)
        self.setCursor(Qt.CursorShape.PointingHandCursor)
        self.setToolTip("Тёмная / светлая тема")
        self._timer = QTimer(self)
        self._timer.timeout.connect(self._step)
    ...
    def paintEvent(self, event):
        ...
        W, H, IW, KD, PAD = self._W, self._H, self._IW, self._KD, self._PAD
        pos = self._anim_pos

        # ── outer pill background ─────────────────────────────────────────────
        pill_bg = self._lerp_color(QColor("#1e1e1e"), QColor("#dde1ec"), pos)
        p.setPen(Qt.PenStyle.NoPen)
        p.setBrush(pill_bg)
        ...
        gx = IW; gw = W - IW * 2
        gy = PAD; gh = H - PAD * 2
        groove_bg = self._lerp_color(QColor("#383838"), QColor("#bcc1ce"), pos)
        p.setBrush(groove_bg)
        p.drawRoundedRect(gx, gy, gw, gh, gh // 2, gh // 2)

        # ── green filled progress arc (left portion of groove) ────────────────
        if pos > 0.01:
            prog_w = int(pos * (gw - KD))
            prog_c = QColor(C_GREEN)
            prog_c.setAlpha(120)
            p.setBrush(prog_c)
            p.drawRoundedRect(gx, gy, prog_w + KD // 2, gh, gh // 2, gh // 2)

        # ── knob ──────────────────────────────────────────────────────────────
        travel = gw - KD - 2
        kx = int(gx + 1 + pos * travel)
        ky = (H - KD) // 2
        # shadow-like darker ring behind knob
        p.setBrush(QColor(0, 0, 0, 40))
        p.drawEllipse(kx - 1, ky - 1, KD + 2, KD + 2)
        ...
        moon_a = 1.0 - pos * 0.5
        p.setOpacity(moon_a)
        p.setFont(QFont("Segoe MDL2 Assets", 13))
        p.setPen(QColor(C_GREEN) if pos < 0.5 else QColor(C_TEXT_DIM))
        p.drawText(0, 0, IW, H, Qt.AlignmentFlag.AlignCenter, "\uE708")

        # ── sun icon (right) — MDL2 \uE706 ────────────────────────────────────
        sun_a = 0.35 + pos * 0.65
        p.setOpacity(sun_a)
        p.setPen(QColor("#f0a500") if pos > 0.5 else QColor(C_TEXT_DIM))
        p.drawText(W - IW, 0, IW, H, Qt.AlignmentFlag.AlignCenter, "\uE706")
...
def make_dashboard_panel() -> tuple["QScrollArea", "StatCard", "StatCard", "QPushButton", "QPushButton"]:
    panel = QWidget()
    ...
    profile_cb.setFixedWidth(160)
    profile_cb.setFont(QFont("Segoe UI", 9))
    ...
    for lbl, val, mn, mx, cur in [
        ...
        vcl.addWidget(SliderRow(lbl, val, mn, mx, cur))
    ...
    for lbl, init in [("Auto Parry", True), ("Auto Sprint", True),
        ...
        gcl.addWidget(ToggleRow(lbl, init))
    gcl.addWidget(ComboRow("Stamina Helper", ["Normal", "Enhanced", "Off"]))
    gcl.addWidget(SliderRow("Ability Cooldown", "0.85", 0, 200, 85))
    gcl.addStretch()
    ...
    ph.addWidget(pl)
    ph.addWidget(ToggleSwitch(initial=True))
    ph.addSpacing(8)
    pe = QLineEdit("42")
    pe.setFixedWidth(56)
    ...
    ncl.addWidget(SliderRow("Packet Loss", "0%", 0, 100, 0))
    ncl.addWidget(ComboRow("Rate Limit", ["Disabled", "1 Mbps", "5 Mbps", "10 Mbps"]))
    ncl.addStretch()
    ...
    for lbl, init in [("Unlock Console", True), ("Remove Fog", True), ("Streamer Mode", False)]:
        mcl.addWidget(ToggleRow(lbl, init))
    mcl.addWidget(ComboRow("Custom Crosshair", ["Crosshair 1", "Crosshair 2", "Off"]))
    mcl.addStretch()
    ...
    return scroll, _cfg_card, _upd_card, _apply_btn_ref[0] if _apply_btn_ref else None, upd
...
class UpdateChecker(QObject):
    """Fetches version.json from UPDATE_CHECK_URL in a QThread."""
    ...
    def check(self):
        try:
            req = urllib.request.Request(
                UPDATE_CHECK_URL,
                headers={"User-Agent": f"DeadlockTweaker/{APP_VERSION}"},
            )
    ...
    def download(self, url: str, dest: str):
        ...
        try:
            ...
            self.finished.emit({"downloaded": dest})
        except Exception as exc:
            self.error.emit(str(exc))
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        outer.setStyleSheet(
            f"#outer {{ background:{C_BG}; border-radius:16px; border:1px solid {C_BORDER}; }}"
        )
        ...
        sidebar.setStyleSheet(
            f"#sidebar {{ background:{C_SIDEBAR}; border-top-left-radius:16px;"
            f"border-bottom-left-radius:16px; border-right:1px solid {C_BORDER}; }}"
        )
        ...
        tw_layout.setContentsMargins(0, 0, 0, 6)
        tw_layout.addWidget(self._theme_toggle, 0, Qt.AlignmentFlag.AlignHCenter)
        sb.addWidget(toggle_wrap)
        ...
        dashboard_panel, self._cfg_card, self._upd_card, apply_btn, upd_btn = make_dashboard_panel()
        if apply_btn:
    ...
    def _toggle_theme(self):
        # Global guard — survives across window recreations
        app = QApplication.instance()
        if getattr(app, "_theme_transitioning", False):
            return
        app._theme_transitioning = True

        new_theme = "light" if _CURRENT_THEME == "dark" else "dark"
        was_maximized = self._win_maximized
        geo = self.geometry()
        page_idx = self._stack.currentIndex()

        # ── 1. Grab screenshot ────────────────────────────────────────────────
        # Always use self.grab() — grabWindow() can hang with certain GPU drivers.
        # For maximized the widget covers the full screen so the grab is identical.
        screen = self.screen() or app.primaryScreen()
        screen_geo = screen.geometry()
        snap = self.grab()
        # Always use the actual window geometry for the overlay —
        # for maximized this is the work-area (excludes taskbar), not full screen_geo
        snap_geo = self.geometry()

        # ── 2. Build opaque overlay ───────────────────────────────────────────
        snap_win = QLabel()
        snap_win.setWindowFlags(
            Qt.WindowType.FramelessWindowHint |
            Qt.WindowType.WindowStaysOnTopHint |
            Qt.WindowType.Tool
        )
        snap_win.setAttribute(Qt.WidgetAttribute.WA_ShowWithoutActivating)
        snap_win.setScaledContents(True)
        snap_win.setPixmap(snap)
        snap_win.setGeometry(snap_geo)
        snap_win.show()
        snap_win.raise_()
        app.processEvents()             # ensure snap is composited before hiding

        # ── 3. Tear down old window ───────────────────────────────────────────
        try:
        ...
        self._watcher_thread.wait(500)  # wait for clean thread exit before destroy
        self._watcher_thread = None
        self.hide()
        self.deleteLater()

        # ── 4. Build new window behind the snapshot ───────────────────────────
        def _build_new():
            new_win = None
            try:
                set_theme(new_theme)
                new_win = MainWindow()
                new_win._switch_page(page_idx)
                # Always restore to exact same geometry (geo = self.geometry() at transition time)
                new_win.setGeometry(geo)
                new_win.show()
                if was_maximized:
                    new_win._win_maximized = True
                    if hasattr(new_win, "_btn_max"):
                        new_win._btn_max.setText("❐")
                app.processEvents()
                snap_win.raise_()

                def _start_fade():
                    anim = QPropertyAnimation(snap_win, b"windowOpacity")
                    ...
                    def _done():
                        snap_win.deleteLater()
                        app._theme_transitioning = False

                    anim.finished.connect(_done)
                    anim.start()
                    new_win._tf_snap_win = snap_win
                    new_win._tf_anim = anim

                QTimer.singleShot(60, _start_fade)
                app._main_win = new_win

            except Exception:
                traceback.print_exc()
                snap_win.deleteLater()
                app._theme_transitioning = False
                # Always ensure a window exists so the app doesn't quit
                if new_win is None:
                    try:
                        set_theme(new_theme)
                        fallback = MainWindow()
                        fallback.setGeometry(geo)
                        fallback.show()
                        if was_maximized:
                            fallback._win_maximized = True
                        app._main_win = fallback
                    except Exception:
                        traceback.print_exc()

        QTimer.singleShot(50, _build_new)
    ...
    def _on_check_update(self):
        if self._pending_download_url:
            ...
            dest = str(Path(__file__).resolve().parent / "DeadlockTweaker_update.exe")
            worker = UpdateChecker()
            thread = QThread(self)
            worker.moveToThread(thread)
            dl_url = self._pending_download_url
            thread.started.connect(lambda: worker.download(dl_url, dest))
    ...
    def _on_update_result(self, data: dict):
        ...
        if _version_tuple(latest) > _version_tuple(APP_VERSION):
            self._pending_download_url = dl_url
            self._upd_btn.setText(f"↓  Download v{latest}")
            ...
            QTimer.singleShot(3000, lambda: self._upd_btn.setText("↓  Check for Updates"))

    def _on_update_error(self, msg: str):
        self._upd_btn.setEnabled(True)
        self._upd_btn.setText("⚠  Check failed — retry")
        self._upd_btn.setToolTip(msg)
        QTimer.singleShot(4000, lambda: self._upd_btn.setText("↓  Check for Updates"))

    def _on_download_result(self, data: dict):
        self._upd_btn.setEnabled(True)
        dest = data.get("downloaded", "")
        if dest:
            self._upd_btn.setText("✓  Downloaded! Click to open")
            self._upd_btn.setToolTip(dest)

            def _open_folder():
                subprocess.Popen(["explorer", "/select,", dest])
            self._upd_btn.clicked.disconnect()
            self._upd_btn.clicked.connect(_open_folder)
        else:
